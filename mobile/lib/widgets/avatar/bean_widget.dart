  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          // 1. Base (Skin)
          _buildLayer(config['skin_color'], 'assets/skins/bean_base_default.png'), 
          
          // 2. Body / Clothes
          if (config['body'] != null)
            _buildLayer(config['body'], null),

          // 3. Head
          if (config['head'] != null)
            _buildLayer(config['head'], null),
  ...
  Widget _mockRender(ShopUserItem item) {
    ...
    if (item.slotType == 'skin_color') {
        ...
        return Container(
          ...
          child: Center(child: Text("^_^\n${item.name}", textAlign: TextAlign.center, style: const TextStyle(fontSize: 8))),
        );
    } 
        ...
        return Positioned(
            ...
            child: Container(
                ...
                child: Center(child: Text(item.name, style: const TextStyle(fontSize: 8))),
            )
        );
    }
        ...
        return Positioned(
            top: 0,
            child: Icon(Icons.star, size: size * 0.4, color: Colors.purple) // Generic Hat
        );
    }
