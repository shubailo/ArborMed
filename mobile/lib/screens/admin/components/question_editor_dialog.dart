...
class _QuestionEditorDialogState extends State<QuestionEditorDialog> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _textControllerEn = TextEditingController(text: widget.question?.text ?? '');
  ...
  AdminQuestion _getLiveQuestion() {
    return AdminQuestion(
      ...
      content: {
        ...
        'link_word': {'en': _linkEnController.text, 'hu': _linkHuController.text},
        'pairs': _matchingGroups.map((g) => {
  ...
  void _addMatchingGroup({String leftE = '', String leftH = '', String rightE = '', String rightH = ''}) {
          ...
          children: [
            ...
            Expanded(
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    const Text("LIVE PREVIEW", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey, fontSize: 10, letterSpacing: 1.2)),
                    const SizedBox(height: 16),
                    Expanded(
                      child: Center(
                        child: FittedBox(
                          ...
                          child: AdminPhonePreview(
                            child: QuestionPreviewCard(
                              ...
                              language: _tabController.index == 0 ? 'en' : 'hu',
                            ),
                          ),
                        ),
                      ),
    ...
    if (_questionType == 'relation_analysis') {
      ...
      if (_matchingGroups.isEmpty) {
        _matchingGroups.add(MatchingPairControllerGroup(
          leftEn: TextEditingController(), leftHu: TextEditingController(), 
          rightEn: TextEditingController(), rightHu: TextEditingController()
        ));
      }
    } else if (_questionType == 'multiple_choice') {
    ...
    try {
      ...
      if (result != null) {
        String stripPrefix(String? text) {
          if (text == null) return "";
          return text.replaceAll('[HU] ', '').replaceAll('[HU]', '')
                     .replaceAll('[EN] ', '').replaceAll('[EN]', '');
        }

        setState(() {
          if (target == 'hu') {
            if (result['explanation'] != null) _explanationControllerHu.text = stripPrefix(result['explanation']);
            if (_questionType == 'relation_analysis') {
              if (result['statement1'] != null) _s1HuController.text = stripPrefix(result['statement1']);
              if (result['statement2'] != null) _s2HuController.text = stripPrefix(result['statement2']);
              if (result['link_word'] != null) _linkHuController.text = stripPrefix(result['link_word']);
            } else if (_questionType == 'matching') {
              if (result['lefts'] != null) {
                final list = result['lefts'] as List;
                for (int i=0; i<list.length && i<_matchingGroups.length; i++) {
                  _matchingGroups[i].leftHu.text = stripPrefix(list[i].toString());
                }
              }
              if (result['rights'] != null) {
                final list = result['rights'] as List;
                for (int i=0; i<list.length && i<_matchingGroups.length; i++) {
                  _matchingGroups[i].rightHu.text = stripPrefix(list[i].toString());
                }
              }
            } else {
              if (result['questionText'] != null) _textControllerHu.text = stripPrefix(result['questionText']);
              if (result['options'] != null) {
                _currentOptionsHu = (result['options'] as List).map((e) => stripPrefix(e.toString())).toList();
              }
            }
          } else {
            if (result['explanation'] != null) _explanationControllerEn.text = stripPrefix(result['explanation']);
            if (_questionType == 'relation_analysis') {
              if (result['statement1'] != null) _s1EnController.text = stripPrefix(result['statement1']);
              if (result['statement2'] != null) _s2EnController.text = stripPrefix(result['statement2']);
              if (result['link_word'] != null) _linkEnController.text = stripPrefix(result['link_word']);
            } else if (_questionType == 'matching') {
              if (result['lefts'] != null) {
                final list = result['lefts'] as List;
                for (int i=0; i<list.length && i<_matchingGroups.length; i++) {
                  _matchingGroups[i].leftEn.text = stripPrefix(list[i].toString());
                }
              }
              if (result['rights'] != null) {
                final list = result['rights'] as List;
                for (int i=0; i<list.length && i<_matchingGroups.length; i++) {
                  _matchingGroups[i].rightEn.text = stripPrefix(list[i].toString());
                }
              }
            } else {
              if (result['questionText'] != null) _textControllerEn.text = stripPrefix(result['questionText']);
              if (result['options'] != null) {
                _currentOptionsEn = (result['options'] as List).map((e) => stripPrefix(e.toString())).toList();
              }
            }
          }
        });
  ...
  Widget _buildTypeSpecificEditor(String lang) {
    switch (_questionType) {
      case 'relation_analysis': return _buildRelationAnalysisEditor(lang);
      case 'matching': return _buildMatchingEditor(lang);
      case 'multiple_choice': return _buildMultipleChoiceEditor(lang);
      case 'true_false': return _buildTrueFalseEditor(lang);
      default: return _buildSingleChoiceEditor(lang);
    }
  }
  ...
  Widget _buildSingleChoiceEditor(String lang) {
    ...
    return DynamicOptionList(
      ...
      onOptionsChanged: (newOpts) => setState(() => isEn ? _currentOptionsEn = newOpts : _currentOptionsHu = newOpts),
      onCorrectIndexChanged: (idx) => setState(() => _correctIndex = idx),
      onAdd: () => setState(() { _currentOptionsEn.add(""); _currentOptionsHu.add(""); }),
      onRemove: (idx) {
        if (_currentOptionsEn.length <= 2) return;
        setState(() {
          _currentOptionsEn.removeAt(idx); _currentOptionsHu.removeAt(idx);
  ...
  Widget _buildMultipleChoiceEditor(String lang) {
    ...
    return Column(
      ...
      children: [
            ...
            child: Row(
              children: [
                Checkbox(
                  ...
                  onChanged: (val) => setState(() => val == true ? _multipleCorrectIndices.add(index) : _multipleCorrectIndices.remove(index)),
                ),
                Expanded(
                  child: TextField(
                    ...
                    onChanged: (val) => setState(() => isEn ? _currentOptionsEn[index] = val : _currentOptionsHu[index] = val),
                    decoration: InputDecoration(labelText: "Option ${index + 1}", border: const OutlineInputBorder(), contentPadding: const EdgeInsets.symmetric(horizontal: 12)),
                  ),
                ),
                IconButton(
                  ...
                  onPressed: () {
                    ...
                    setState(() {
                      _currentOptionsEn.removeAt(index); _currentOptionsHu.removeAt(index); _multipleCorrectIndices.remove(index);
        ...
        TextButton.icon(onPressed: () => setState(() { _currentOptionsEn.add(""); _currentOptionsHu.add(""); }), icon: const Icon(Icons.add), label: const Text("Add Option")),
      ],
    );
  }

  Widget _buildRelationAnalysisEditor(String lang) {
    bool s1 = false; bool s2 = false; bool link = false;
    ...
    return Column(
      children: [
        ...
        const SizedBox(height: 16),
        _buildRAOption(!s1, s1, "Statement 1 is TRUE", (val) => _correctIndex = calculateIndex(val, s2, link)),
        const SizedBox(height: 12),
        _buildRAOption(!s2, s2, "Statement 2 is TRUE", (val) => _correctIndex = calculateIndex(s1, val, link)),
        const SizedBox(height: 12),
        Opacity(
          opacity: (s1 && s2) ? 1.0 : 0.5,
          child: _buildRAOption(!link, link, "Connection / Link Exists (Because...)", (val) { if (s1 && s2) _correctIndex = calculateIndex(s1, s2, val); }, isLink: true),
        ),
      ],
    );
  }

  Widget _buildRAOption(bool toggle, bool active, String label, Function(bool) onChanged, {bool isLink = false}) {
    return InkWell(
    ...
    return Column(
      ...
      children: [
        ...
        ..._matchingGroups.asMap().entries.map((entry) {
          final idx = entry.key; final group = entry.value;
          return Padding(
        ...
        TextButton.icon(onPressed: () => _addMatchingGroup(), icon: const Icon(Icons.add), label: const Text("Add Pair")),
      ],
    );
  }

  Future<void> _translateField({required String from, required String to, required TextEditingController sourceCtrl, required TextEditingController targetCtrl}) async {
  ...
  void _save() async {
    ...
    if (_questionType == 'relation_analysis') {
      ...
      final List<Map<String, dynamic>> pairs = []; final Map<String, String> correctMap = {};
      for (var group in _matchingGroups) {
        pairs.add({'left': {'en': group.leftEn.text, 'hu': group.leftHu.text}, 'right': {'en': group.rightEn.text, 'hu': group.rightHu.text}});
        if (group.leftEn.text.isNotEmpty) {
          correctMap[group.leftEn.text] = group.rightEn.text;
        }
      }
      contentPayload['pairs'] = pairs; correctAnswerPayload = correctMap;
    } else if (_questionType == 'multiple_choice') {
    ...
    final payload = {
      'question_text_en': _textControllerEn.text, 'question_text_hu': _textControllerHu.text,
      'options_en': _currentOptionsEn, 'options_hu': _currentOptionsHu,
      'explanation_en': _explanationControllerEn.text, 'explanation_hu': _explanationControllerHu.text,
      'correct_answer': correctAnswerPayload, 'question_type': _questionType,
      'topic_id': _selectedTopicId, 'bloom_level': _bloomLevel, 'content': contentPayload,
    };
