...
class _ECGEditorDialogState extends State<ECGEditorDialog> {
  ...
  Future<void> _save() async {
    ...
    if (_selectedDiagnosisId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please select a diagnosis")));
      return;
    }
                ...
                children: [
                   Row(
                     children: [
                       ...
                       Text(
                        widget.ecgCase == null ? "New ECG Case (7+2 Steps)" : "Edit Case #${widget.ecgCase!.id}", 
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Expanded(child: Column(
                          children: [
                            _buildDropdown("difficulty", "Difficulty", _difficulty, difficulties, (v) => _difficulty = v, Icons.signal_cellular_alt),
                            ...
                            Consumer<StatsProvider>(
                              builder: (ctx, stats, _) => DropdownButtonFormField<int>(
                                ...
                                isExpanded: true,
                                items: stats.ecgDiagnoses.map((d) => 
                                  DropdownMenuItem(value: d.id, child: Text("${d.code} - ${d.nameEn}"))
                                ).toList(),
                                onChanged: (val) {
                                  setState(() => _selectedDiagnosisId = val);
                                  if (val != null) {
                                     final d = stats.ecgDiagnoses.firstWhere((e) => e.id == val);
                      ...
                      _buildSectionHeader("0. Patient History (Optional)", Icons.history_edu),
                      const SizedBox(height: 16),
                      TextFormField(
                        controller: _historyController,
                        maxLines: 2,
                        decoration: _getDecoration("history", "Patient Signalment / History").copyWith(
                    ...
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                         Expanded(flex: 3, child: _buildDropdown("rhythm.regularity", "Regularity", _rhythmRegularity, regularityOpts, (v) { _rhythmRegularity = v; _markEdited('rhythm.regularity'); }, Icons.linear_scale)),
                         const SizedBox(width: 12),
                         Expanded(flex: 3, child: _buildDropdown("rhythm.ratio", "Ratio", _conductionRatio, conductionOpts, (v) { _conductionRatio = v; _markEdited('rhythm.ratio'); }, Icons.compare_arrows)),
                         const SizedBox(width: 12),
                         Expanded(flex: 4, child: _autofillWrapper(
                              "rhythm.sinus",
                              InputDecorator(
                                decoration: _getDecoration("rhythm.sinus", "Sinus Rhythm?").copyWith(
                                ...
                                child: Row(
                                  children: [
                                    Transform.scale(
                                      ...
                                      child: Checkbox(
                                        value: _isSinus, 
                                        onChanged: (v) { setState(() => _isSinus = v!); _markEdited('rhythm.sinus'); },
                                        materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                                      ),
                                    ),
                                    const Expanded(
                                      child: Text("P before QRS, positive in II", 
                    ...
                    const SizedBox(height: 16),
                    _buildDropdown("axis", "Heart Axis", _axis, axisList, (v) { _axis = v; _markEdited('axis'); }, Icons.compass_calibration),
                    const SizedBox(height: 24),
                    ...
                    const SizedBox(height: 16),
                    Row(children: [
                       Expanded(child: _buildDropdown("pwave.morph", "Morphology", _pWaveMorph, pMorphs, (v) { _pWaveMorph = v; _markEdited('pwave.morph'); }, Icons.tune)),
                       const SizedBox(width: 16),
                       Expanded(child: _buildDropdown("pwave.enlargement", "Atrial Enlargement", _atrialEnlargement, atrialSizes, (v) { _atrialEnlargement = v; _markEdited('pwave.enlargement'); }, Icons.zoom_out_map)),
                    ]),
                    const SizedBox(height: 24),
                    ...
                    const SizedBox(height: 16),
                    Row(children: [
                       Expanded(child: _buildDropdown("qrs.hypertrophy", "Hypertrophy", _hypertrophy, hypertrophyOpts, (v) { _hypertrophy = v; _markEdited('qrs.hypertrophy'); }, Icons.line_weight)),
                       const SizedBox(width: 16),
                       Expanded(child: _buildDropdown("qrs.bbb", "Bundle Branch Block", _bbb, bbbOpts, (v) { _bbb = v; _markEdited('qrs.bbb'); }, Icons.timeline)),
                    ]),
                    const SizedBox(height: 16),
                    _buildDropdown("qrs.qwaves", "Pathological Q Waves", _qWaves, qWaveOpts, (v) { _qWaves = v; _markEdited('qrs.qwaves'); }, Icons.priority_high),
                    const SizedBox(height: 24),

                    // --- STEP 7: ST-T MORPHOLOGY ---
                    _buildSectionHeader("7. ST-T Morphology", Icons.trending_up),
                    const SizedBox(height: 16),
                    Row(children: [
                       Expanded(child: _buildDropdown("st.ischemia", "Ischemia/Infarction", _ischemia, ischemiaOpts, (v) { _ischemia = v; _markEdited('st.ischemia'); }, Icons.warning_amber)),
                       const SizedBox(width: 16),
                       Expanded(child: _buildDropdown("st.twave", "T-Wave", _tWave, tWaveOpts, (v) { _tWave = v; _markEdited('st.twave'); }, Icons.waves)),
                    ]),
                    const SizedBox(height: 24),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          SwitchListTile(
                            title: const Text("Include Step +2: Management & Urgency?"),
                          ...
                          if (_includeManagement) ...[
                            const SizedBox(height: 16),
                            _buildDropdown("management.urgency", "Urgency Level", _urgency, urgencyOpts, (v) => _urgency = v, Icons.notification_important),
                            const SizedBox(height: 16),
                            TextFormField(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isUploading 
                    ...
                    : const Text("Save 7+2 Case", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white)),
                ),
              ),
            )
          ],
      ...
      children: [
        ...
        Positioned(
          ...
          child: Container(
            ...
            child: const Icon(Icons.auto_awesome, size: 14, color: Colors.green),
          ),
        )
      ],
    );
  ...
  Widget _buildDropdown(String key, String label, String value, List<String> items, Function(String) onChanged, IconData icon) {
    return _autofillWrapper(key, DropdownButtonFormField<String>(
      ...
      items: items.map((r) => DropdownMenuItem(
        value: r, 
        child: Text(r, style: const TextStyle(fontSize: 14), overflow: TextOverflow.ellipsis)
      )).toList(),
                 ...
                 child: Stack(
                   ...
                   children: [
                     Image.network(
                       ...
                       fit: BoxFit.contain,
                       errorBuilder: (ctx, _, __) => const Center(child: Icon(Icons.broken_image, size: 40, color: Colors.grey)),
                     ),
                     Positioned(
                       bottom: 8, right: 8,
                       child: Container(
         ...
         return InkWell(
           ...
           child: InputDecorator(
             ...
             child: _secondaryDiagnosesIds.isEmpty 
               ? const Text("None (Tap to add)", style: TextStyle(color: Colors.grey, fontSize: 14))
               : Wrap(
                   spacing: 6,
                   runSpacing: 6,
