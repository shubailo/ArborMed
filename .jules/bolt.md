...
## 2024-05-20 - Replace .any + .firstWhere with .where().firstOrNull
**Learning:** Using `.any()` followed by `.firstWhere()` on the same condition iterates over the collection twice, causing O(2N) complexity. In Dart 3, `.firstOrNull` is natively available on Iterables.
**Action:** Replaced a `.any()` + `.firstWhere()` combination in `apps/student_app/lib/features/shop/widgets/contextual_shop_sheet.dart` with `.where().firstOrNull` to evaluate the iterable only once and improve performance. Also refactored unsafe `.firstWhere()` calls in `ecg_editor_dialog.dart` to use `.where().firstOrNull` to avoid `StateError`s.
