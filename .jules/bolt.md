...
## 2025-02-25 - Pre-calculating Sets to prevent O(N*M) local data synchronization linear scans in UI
**Learning:** Using `.any(...)` on a list of local inventory items inside a `GridView.builder` loop that iterates over catalog items creates an O(N*M) time complexity bottleneck.
**Action:** Always pre-process the local items list into `Set<int>` for O(1) lookups before the list builder, and use `.contains()` to check for matches without introducing O(N) array scanning.
