...
## 2026-04-24 - Pre-building Hash Maps for O(N*M) loop elimination
**Learning:** Using `.firstWhere` or `.any` on a list of local database items inside a `.map` loop that iterates over a catalog creates an O(N*M) time complexity bottleneck. In `shop_screen.dart`, scanning `provider.inventory` for each item in `GridView.builder` causes severe UI stuttering as the catalog and user inventory grow.
**Action:** Always pre-process lists into Sets (e.g., `Set<int>`) for O(1) lookups *before* passing them into the `itemBuilder` of lists/grids, ensuring array scans inside loops are eliminated.
