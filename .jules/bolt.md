...
## 2024-07-28 - Prevent O(N*M) list rendering lag in Flutter
**Learning:** Using `.any()` or `.firstWhere()` inside a Flutter `itemBuilder` for `GridView` or `ListView` causes severe O(N*M) performance degradation during scrolling as the list of items grows.
**Action:** Always precompute the derived state into an O(1) `Set` or `Map` before the list rendering callback to ensure lightning-fast scrolling.
