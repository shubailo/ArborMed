...
## 2024-06-14 - Optimize redundant list traversals
**Learning:** The Dart codebase had instances of redundant list traversals: using `.any()` to check for an item's existence followed by `.firstWhere()` to retrieve it (O(2N)).
**Action:** Combine these into a single O(N) pass using `.where(...).firstOrNull`.
