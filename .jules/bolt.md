...
## 2025-02-24 - Pre-building Sets for O(N*M) Widget Builders
**Learning:** Using `.any()` to scan a list inside the `itemBuilder` of a Flutter `GridView.builder` causes an O(N*M) time complexity bottleneck. Because the builder executes on scroll for every newly visible item, this linear search causes significant UI stuttering and frame drops as the user's inventory grows.
**Action:** Always pre-compute derived state (such as equipped item IDs) into a `Set` or `Map` for O(1) lookups *outside* of the `GridView.builder` / `ListView.builder` before the builder callback, and pass that O(1) data structure to the item rendering function.
