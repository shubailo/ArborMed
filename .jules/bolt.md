...
## 2025-06-09 - Optimizing Double List Scans in Widgets
**Learning:** Calling `.any()` followed by `.firstWhere()` on the same list condition causes redundant O(N) scans. When done inside the `build()` method of frequently updated widgets, it causes unnecessary UI thread load.
**Action:** Always combine `.any()` and `.firstWhere()` into a single `.where(...).firstOrNull` call.
