...
## 2024-07-16 - O(N*M) Lookup Issue inside Builders
**Learning:** Found an O(N*M) look up issue inside a scrollable GridView.builder in `WardrobeSheet` which was fixed. I noticed a similar pattern in `ShopScreen` (`apps/student_app/lib/features/shop/screens/shop_screen.dart`), where the `_buildShopItemV2` calls `Provider.of<ShopProvider>(context).inventory.any(...)` inside the `itemBuilder` of a `GridView.builder`. `inventory.any` runs in O(M) time, resulting in O(N*M) per render cycle as user scrolls. Pre-computing this into an O(1) hash map fixes this performance issue.
**Action:** Always precompute derived state (e.g. Map/Set lookup) outside of `ListView.builder` / `GridView.builder` instead of calling `Provider.of` or searching a list directly inside the builder loop to prevent scroll stuttering.
