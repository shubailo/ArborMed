...
## 2026-04-24 - Avoiding Dual Iterations in Collections
**Learning:** Using `.any()` to check for the existence of an item followed immediately by `.firstWhere()` to extract it causes the code to iterate over the collection twice, resulting in O(2N) time complexity.
**Action:** Use `.where(...).firstOrNull` instead to evaluate the iterable only once, extracting the item while simultaneously determining its existence (by checking for `null`).
