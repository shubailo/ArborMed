...
## 2024-05-24 - O(N*M) Array Scan in GridView.builder
**Learning:** Using `.any()` inside Flutter `GridView.builder` causes O(N*M) lag as it runs a list scan for every built element on scroll.
**Action:** Pre-compute derived state into a `Set` or `Map` for O(1) lookups before the builder callback.
