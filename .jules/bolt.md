...
## 2024-07-17 - O(N*M) Provider Inventory Lookups in Flutter GridView/ListView Builders
**Learning:** Checking for state (e.g., if an item is equipped in the inventory via `inventory.any(...)`) inside the `itemBuilder` callback of a `GridView.builder` causes an `O(N)` scan for *every* built item (`M`). This degrades scrolling performance to `O(N*M)` because the grid reconstructs children during scrolling. Also, calling `Provider.of<T>(context)` inside a nested component's build method executes an `O(D)` tree traversal, multiplying the cost.
**Action:** Pre-compute derived state into a `Set` (e.g., `equippedItemIds = inventory.where(...).map(...).toSet()`) *before* the `builder` runs, providing `O(1)` lookups for each child. Pass the derived boolean explicitly into the child widget rather than having the child fetch the Provider itself, avoiding unnecessary `O(D)` tree traversals on rebuild.
