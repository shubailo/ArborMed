...
## 2024-07-12 - Shop Grid View Rendering O(N*M)
**Learning:** In Flutter, `GridView.builder` is rebuilt for each item as it scrolls. If the `itemBuilder` relies on a provider state that performs a list scan via `.any()` or `.firstWhere()` over another collection, it cascades into O(N*M) performance, leading to scroll stutter on large grids.
**Action:** Always precompute derived state (e.g., equipped items) into an O(1) `Set` or `Map` lookup inside the `ChangeNotifier` state setup (e.g., `_loadInventoryFromLocal`) before it's read by builder callbacks. Remember to explicitly comment when a shared method inherently rebuilds this cache during mutations.
