...
## 2025-02-24 - Pre-building Hash Sets to prevent O(N*M) loop elimination in Flutter GridViews
**Learning:** Using `.any` on a list of local inventory items inside the `itemBuilder` of a `GridView.builder` creates an O(N*M) time complexity bottleneck. Because the builder runs repeatedly on scroll, it causes UI stutters.
**Action:** Always pre-process list data into Hash Maps or Sets (e.g., `Set<int>`) for O(1) lookups *before* returning the main UI tree or before passing it to the builder.
