...
## 2025-05-24 - Prototype Pollution and Array DoS in req.query
**Vulnerability:** Controller parameters extracted from `req.query` (like `sortBy` and `order`) were assumed to be strings. If a user provided multiple parameters with the same name (e.g., `?order=desc&order=asc`), Express parses them as an array. Calling string methods like `.toUpperCase()` on an array causes an unhandled `TypeError`, leading to a Denial of Service. Furthermore, `sortBy` was used directly as a key in `sortMap[sortBy]`, which could be exploited for prototype pollution by passing keys like `toString`.
**Learning:** `req.query` parameters are fundamentally untrusted and can be arrays or objects depending on the framework's query parser. Direct access to objects using untrusted keys is a vector for prototype pollution.
**Prevention:** Always explicitly validate that expected string parameters from `req.query` are indeed strings (`typeof === 'string'`) before calling string methods. Use `Object.prototype.hasOwnProperty.call(map, key)` instead of direct property access when checking for valid keys.
