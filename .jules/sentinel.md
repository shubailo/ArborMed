...
**Learning:** Checking authorization conditions sequentially in application code after fetching rows is highly inefficient and vulnerable to scale-based attacks, especially on bulk endpoints.
**Prevention:** Push authorization and ownership checks directly into the database using efficient, negated SQL queries (e.g., finding the first unauthorized row with `LIMIT 1`).
