...
## 2025-05-23 - Prototype Pollution and TypeError in Express Query Params
**Vulnerability:** The `adminGetQuestions` controller used `!sortMap[sortBy]` to validate the `sortBy` query parameter. An attacker could pass `?sortBy=toString` which evaluates to truthy (the `toString` function on `Object.prototype`), bypassing the validation but causing unexpected behavior or errors later. Furthermore, if `order` was passed as an array (`?order=ASC&order=DESC`), Express would parse it as an Array object, causing a `TypeError` crash on `.toUpperCase()`, leading to Denial of Service.
**Learning:** `req.query` parameters in Express can be unexpected types like arrays (if duplicated) or malicious string values that access inherited prototype properties.
**Prevention:** Always validate that query parameters that invoke string methods (like `.toUpperCase()`) are strictly of type `'string'`. Use `Object.prototype.hasOwnProperty.call(map, key)` instead of direct access (`map[key]`) to safely check for keys in mapping objects, preventing prototype pollution bypasses.
