...
## 2024-05-14 - Express Query Array/Prototype Pollution DoS
**Vulnerability:** Express `req.query` parses duplicate parameters as arrays and parses standard strings as objects with prototypes. Directly using string methods like `.toUpperCase()` on `req.query.order` without type checking causes uncaught `TypeError` exceptions if the client sends an array (`?order=ASC&order=DESC`), leading to an unhandled promise rejection and server crash (DoS). Furthermore, directly accessing properties using a user-controlled string like `sortMap[req.query.sortBy]` allows prototype pollution bypasses if the attacker sends `?sortBy=toString` or `__proto__`.
**Learning:** Never assume query parameters from Express are strings. Always validate types before applying string-specific methods. Do not rely on truthiness or undefined checks for object properties populated by user input.
**Prevention:** Explicitly check types (e.g., `typeof order === 'string'`). For key lookups based on user input, strictly enforce the use of `Object.prototype.hasOwnProperty.call(map, key)` to prevent prototype chain traversal.
