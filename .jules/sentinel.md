...
## 2024-03-09 - [Express Query Param Array TypeError / Prototype Pollution]
**Vulnerability:** The `adminGetQuestions` controller directly called string methods (like `.toUpperCase()`) on Express `req.query` parameters without validating their type. It also used untrusted `req.query` values directly as keys for `sortMap[sortBy]`. This opened the application to Denial of Service (DoS) crashes if an array was passed (e.g., `?order=ASC&order=DESC`) resulting in a `TypeError`, and potentially allowed prototype pollution if keys like `__proto__` or `toString` were passed.
**Learning:** In Express, `req.query` parameters are NOT guaranteed to be strings. Duplicate query keys are automatically parsed into arrays, and any value can be used as an object key. Direct invocation of string methods or direct property access on maps using untrusted input is unsafe.
**Prevention:** Always explicitly validate that query parameters are strings using `typeof param === 'string'` before performing string operations. When looking up values in a map based on query input, strictly use `Object.prototype.hasOwnProperty.call(map, key)` to prevent prototype pollution and ensure the key actually exists on the object itself.
