...
## 2025-05-24 - Express req.query Type Confusion and Prototype Pollution
**Vulnerability:** In `adminQuestionController.js` (and potentially other controllers), `req.query.sortBy` was used to directly index `sortMap` (`sortMap[sortBy]`), leading to prototype pollution (e.g. `?sortBy=toString`). Additionally, `req.query.order.toUpperCase()` was called without validating the type of `order`. If an attacker provided an array (e.g., `?order=a&order=b`), `toUpperCase` would be called on an array, causing a `TypeError` and crashing the Node.js process (DoS).
**Learning:** In Express.js, `req.query` properties can unexpectedly be arrays or objects depending on the query string structure (e.g., duplicate keys or nested keys). Calling string methods directly on them without `typeof` validation is unsafe and can lead to DoS. Using untrusted input as object keys without `hasOwnProperty` is also unsafe.
**Prevention:** Always validate that a query parameter is a `string` (e.g., `typeof order === 'string'`) before calling string methods. Use `Object.prototype.hasOwnProperty.call(map, key)` when dynamically accessing map properties using user input.
