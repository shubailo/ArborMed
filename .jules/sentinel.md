...
## 2025-05-24 - Prototype Pollution and DoS via Array Query Params
**Vulnerability:** The application was susceptible to unhandled `TypeError` crashes (a Denial of Service vulnerability) when `req.query.order` or `req.query.sortBy` was passed as an array via duplicated query parameters (e.g. `?order=DESC&order=ASC`). Calling `.toUpperCase()` on an array throws an error. The application was also susceptible to prototype pollution by accessing an object property via `req.query` without verifying the key (e.g. `sortMap[sortBy]`), allowing arbitrary access like `?sortBy=toString`.
**Learning:** `req.query` values cannot be trusted to be strings, as duplicate keys are automatically parsed into arrays by Express. Furthermore, direct property access on objects via untrusted query params opens the door to prototype pollution.
**Prevention:** Always validate that optional `req.query` parameters are strings (e.g., `typeof order === 'string'`) before calling string methods. Always validate keys on objects using `Object.prototype.hasOwnProperty.call()` instead of direct bracket access.
