...
## 2025-05-23 - Prototype Pollution & DoS via Query Arrays
**Vulnerability:** In Express.js applications, `req.query` properties can be arrays if the client provides the same query parameter multiple times (e.g., `?order=asc&order=desc`). If the value is blindly assumed to be a string and methods like `.toUpperCase()` are called on it, an unhandled `TypeError` crash occurs, leading to Denial of Service (DoS). Furthermore, using untrusted query keys directly on objects (e.g., `sortMap[sortBy]`) allows for prototype pollution (e.g., `?sortBy=__proto__`).
**Learning:** Never assume query parameters are strings. Validating properties directly on plain objects can be subverted via prototype methods.
**Prevention:** Always validate that query parameters are strings before operating on them (e.g., `typeof order === 'string'`). For key lookups, use `Object.prototype.hasOwnProperty.call(map, key)` to safely check for keys.
