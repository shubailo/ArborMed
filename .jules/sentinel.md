...
## 2025-05-24 - Unvalidated req.query types leading to DoS and Prototype Pollution
**Vulnerability:** The `sortBy` and `order` parameters in `req.query` were accessed without type checking. If a user provided an array for `order` (e.g., `?order=ASC&order=DESC`), `order.toUpperCase()` crashed the process. Furthermore, `sortMap[sortBy]` allowed prototype pollution via `?sortBy=toString`, injecting native function definitions into the SQL query.
**Learning:** Express's query string parser creates arrays for duplicate parameters and does not protect against prototype lookups. Always validate that query inputs are strictly strings and use safe dictionary lookups (e.g., `Object.prototype.hasOwnProperty.call`).
**Prevention:** Always check `typeof param === 'string'` before calling string methods and use `Object.prototype.hasOwnProperty.call(map, key)` when mapping untrusted keys to values.
