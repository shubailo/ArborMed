...
CREATE TABLE IF NOT EXISTS security_audits (
    ...
    user_id INTEGER,
    admin_id INTEGER, -- If an admin did something to a user
...
-- Enable RLS (Security Gold Standard)
ALTER TABLE security_audits ENABLE ROW LEVEL SECURITY;

-- Only admins should ever see these logs
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'security_audits' AND policyname = 'admin_audit_access') THEN
