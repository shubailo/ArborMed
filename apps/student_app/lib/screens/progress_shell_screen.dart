...
import 'package:student_app/features/progress/domain/entities/progress.dart';
import 'package:student_app/features/progress/presentation/widgets/activity_trends_panel.dart';

class ProgressShellScreen extends ConsumerWidget {
  const ProgressShellScreen({super.key});
      ...
      body: progressAsync.when(
        ...
        data: (progress) => ListView(
          ...
          children: [
            ...
            const ActivityTrendsPanel(),
            const SizedBox(height: 32),
            if (progress.topics.isEmpty)
              const Center(
                child: Text(
                  'Start studying to see your progress ladder fill up.',
                ),
              )
            else
              ...progress.topics.map(
                (topic) => TopicProgressCard(topic: topic),
              ),
          ],
        ),
      ),
...
class TopicProgressCard extends StatelessWidget {
  ...
  String _getBadgeText(String badge) {
    switch (badge) {
      case 'FOUNDATION':
        return 'Foundations in place';
      case 'APPLICATION':
        return 'You can apply this topic';
      case 'ADVANCED':
        return 'You analyze and evaluate confidently';
      case 'EXPERT':
        return 'You have mastered even the highest levels';
      default:
        return 'No badge yet';
    }
  }
