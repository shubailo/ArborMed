...
class _ContextualShopSheetState extends State<ContextualShopSheet> {
  ...
  Widget _buildDetailActions(ShopProvider provider) {
    ...
    // ⚡ Bolt: Single Pass Lookup Optimization
    // What: Combined `.any` and `.firstWhere` into a single `.where().firstOrNull` call.
    // Why: Using `.any` followed by `.firstWhere` traverses the list twice unnecessarily, which is O(N) but doing it twice per rebuild causes overhead.
    // Impact: Halves the time spent searching the user inventory in this build method.
