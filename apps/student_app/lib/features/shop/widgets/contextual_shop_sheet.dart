...
class _ContextualShopSheetState extends State<ContextualShopSheet> {
  ...
  Widget _buildDetailActions(ShopProvider provider) {
    ...
    // ⚡ Bolt: Inventory Lookup Optimization
    // What: Replaced an O(2N) lookup (.any followed by .firstWhere) with a single pass (.firstOrNull).
    // Why: Prevents iterating through the inventory list twice, reducing CPU usage during detailed rendering.
    // Impact: Halves the loop iterations needed to find a placed user item.
