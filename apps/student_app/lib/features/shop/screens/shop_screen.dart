...
class _ShopScreenState extends State<ShopScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // ⚡ Bolt: Placed Items O(1) Lookup Optimization
    // 💡 What: Pre-compute placed items into a `Set<int>` instead of searching via `.any` in O(N*M) during scrolling.
    // 🎯 Why: A GridView.builder builds elements on scroll. Running `.any` on `provider.inventory` for every element causes UI stutters.
    // 📊 Impact: Reduces lookup complexity from O(M) per item to O(1), improving scroll performance significantly.
    // 🔬 Measurement: Removed nested loop overhead inside `_buildShopItemV2` called by `itemBuilder`.
