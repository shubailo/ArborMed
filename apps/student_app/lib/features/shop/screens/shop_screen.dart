...
import '../../room/widgets/cozy_room_renderer.dart';
import 'package:arbor_med/widgets/cozy/cozy_button.dart';
import 'package:arbor_med/widgets/cozy/paper_texture.dart';
...
class _ShopScreenState extends State<ShopScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // What: Pre-compute placed inventory items into a Set to avoid O(N*M) lookups during GridView rendering.
    // Why: GridView.builder renders items on scroll. Running `any` on inventory for each catalog item causes UI stutters.
    // Impact: Reduces lookup complexity from O(M) to O(1) per catalog item.
    final equippedItemsSet = {
      for (var item in provider.inventory)
        if (item.isPlaced) item.itemId,
    };
  ...
  Widget _buildShopItemV2(
    ShopItem item,
    int currentCoins,
    Set<int> equippedItemsSet,
  ) {
    final isEquipped = item.isOwned && equippedItemsSet.contains(item.id);
