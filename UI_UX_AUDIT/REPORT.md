...
## 6. Code-Level Audit Findings
*   **Accessibility:** Custom interactive elements built with `GestureDetector` (such as the 'Like Room' button wrapped in a `Tooltip` in `CozyActionsOverlay`) lack explicit semantic wrappers (`Semantics(button: true)`). While `Tooltip` provides some labeling, `Semantics` should be used to properly expose custom interactive elements as buttons to screen readers.
*   **Color Theme Adherence:** While the app primarily uses `CozyTheme`, there are several instances of hardcoded colors that break the centralized theming system. For example, `Color(0xFF4A4A4A)` is used for the visitor tag background in `room_screen.dart`, and `Color(0xFFFFFDF5)` / `Color(0xFF8CAA8C)` are hardcoded in the `_showLeaveNoteDialog` method within the same file. These should be replaced with theme variables from `CozyTheme`.
*   **State-driven Visual Noise:** The `QuizSessionScreen` utilizes a `PulseNotifier` and continuously adds new `CoinParticle` widgets into a state list (`_coinParticles`). During rapid interactions, this overlay-heavy approach can cause visual clutter alongside the `ConfettiOverlay` and `QuizFeedbackOverlay`.
