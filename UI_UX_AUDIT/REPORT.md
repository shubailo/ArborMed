...
*   **Visibility of System Status:**
    ...
    *   *Negative:* When loading large question banks locally via `Drift`, the `QuizLoadingScreen` lacks sufficient granular progress communication.
...
*   **Aesthetic and Minimalist Design:**
    *   *Negative:* The isometric room, while central to the "Cozy Competence" theme, is computationally and visually heavy when running beneath intensive tasks like timed ECG practice or Duel Mode. Furthermore, empty states in the Shop lack personality.

### 2.2 Content and Architecture
*   **Information Architecture:** The navigation heavily relies on contextual sheets (e.g., `ContextualShopSheet`, `ClinicDirectorySheet`) invoked from a 3D hub. While immersive, it obscures direct paths to high-yield actions.
...
*   **Interactivity:** Interactive elements lack sufficient tactile feedback natively. Although `CozyHaptics` and `AudioProvider` are integrated, their application is inconsistent across standard Flutter widgets like `GestureDetector` that aren't wrapped in `CozyButton`.
...
*   *Issue:* Running the 3D/Isometric `FloatingMedicalIcons` behind the `QuizSessionScreen` increases visual noise and drains battery.
*   *Solution:* Implement a solid, themed background (e.g., `#F4F1ED` with subtle watermark patterns) for the Quiz Session by removing `FloatingMedicalIcons`. The room should pause or unload when entering a deep focus state.
...
*   *Solution:* Audit all `GestureDetector` widgets in the app (e.g. in `ContextualShopSheet`). Ensure any button or card that changes state triggers a `CozyHaptics.mediumTap()` along with the corresponding audio SFX.
*   *Rationale:* Essential for the "Cozy" tactile feel the brand promises.

**Low Priority: Refine "Shop" Empty States**
*   *Issue:* If the shop catalog fails to load (`_buildErrorView` in `ShopScreen`), the error state is generic.
*   *Solution:* Add a themed illustration (e.g., `Icons.warning_amber_rounded`) and a more playful copy ("Our supply truck got a flat tire! Re-fetch Storage").
...
*   **Recommendation:**
    ...
    *   **Subdomain Strategy:**
        ...
        *   `admin.arbormed.com`: Dedicate this subdomain to the Admin portal to keep administrative traffic isolated and secure.
