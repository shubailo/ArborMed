...
*   **Visibility of System Status:**
    *   *Positive:* Gamification elements like coins (Stethoscope currency) and streaks update dynamically.
...
*   **Match Between System and Real World:**
    *   *Positive:* The Medical Shop and terminology ("Clinic") cleverly match the medical student reality while keeping it playful.
...
### 2.3 Visual Design
*   **Color & Typography:** The pastel palette (Sage greens `#8CAA8C`, warm clays `#C48B76`, creamy backgrounds `#FDFCF8`) strictly adheres to the "Cozy Competence" guidelines. The use of `Figtree` is modern and readable.
*   **Interactivity:** Interactive elements lack sufficient tactile feedback natively. Although `CozyHaptics` and `AudioProvider` are integrated, their application is inconsistent across standard Flutter widgets like standard `ListTile` or `GestureDetector` that aren't wrapped in consistent animated wrappers.
...
*   **Recommendation:**
    *   **Primary Domain:** `med-buddy-lrri.onrender.com` is currently the api base url.
    *   **Subdomain Strategy:**
        *   Host the API at `med-buddy-lrri.onrender.com` as it is currently defined in `apps/student_app/lib/services/api_service.dart`.
        *   Establish a primary domain like `arbormed.ai` and use `app.arbormed.ai` for web hosting.
        *   Set up `admin.arbormed.ai` to isolate the admin application.
...
2.  **Interactive "Review" Clinic:**
    *   Instead of a standard list for reviewing missed questions, populate a specific area of the user's room (e.g., a "Clinical Bay") where they physically click to review past mistakes.
3.  **Collaborative Study Rooms (Social Extension):**
    *   Allow players to invite friends to their custom isometric room. While hanging out, they can trigger synchronous duels using the existing `socket.io` duel infrastructure, but in a cooperative mode.
