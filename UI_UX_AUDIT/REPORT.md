...
*   **Visibility of System Status:**
    ...
    *   *Negative:* When loading large question banks locally via `Drift`, the loading screens (e.g., in `quiz_loading_screen.dart`) lack sufficient granular progress communication, sometimes appearing frozen.
...
*   **Information Architecture:** The navigation heavily relies on contextual sheets (e.g., `contextual_shop_sheet.dart`) invoked from a 3D hub. While immersive, it obscures direct paths to high-yield actions (like "Resume Last Study Session").
*   **Content Organization:** The Quiz interface (`quiz_session_screen.dart`) correctly places the stem (question text) in prominent focus, but the answer option hit targets and feedback overlays (`quiz_feedback_overlay.dart`) occasionally overlap with floating decorative particles, creating visual clutter during the crucial "learning from mistakes" phase.

### 2.3 Visual Design
*   **Color & Typography:** The pastel palette (Sage greens `#8CAA8C`, warm browns `#C48B76`, creamy backgrounds `#FDFCF8`) strictly adheres to the "Cozy Competence" guidelines.
*   **Interactivity:** Interactive elements lack sufficient tactile feedback natively. Although haptics (`haptic_service.dart`) and audio (`audio_provider.dart`) are integrated, their application is inconsistent across standard Flutter widgets like standard `ListTile` or `GestureDetector` that aren't wrapped in specialized buttons.
...
*   *Issue:* Running the 3D/Isometric room beneath the quiz session (`quiz_session_screen.dart`) increases visual noise and drains battery.
