...
*   **Visibility of System Status:**
    ...
    *   *Negative:* When loading locally via `Drift`, the quiz loading phase lacks sufficient granular progress communication, sometimes appearing frozen.
...
*   **Consistency and Standards:**
    *   *Negative:* The mixture of modal/overlay interactions vs. full-screen routing for the room system creates navigation confusion. For instance, the transition from Dashboard to Quiz sometimes layers heavy 3D elements behind the quiz, distracting from the cognitive load of studying.
...
*   **Content Organization:** The Quiz interface correctly places the stem (question text) in prominent focus, but the answer option hit targets and feedback overlays occasionally overlap with floating decorative particles (`coin_particle.dart`), creating visual clutter during the crucial "learning from mistakes" phase.
...
*   **Interactivity:** Interactive elements lack sufficient tactile feedback natively. Although haptics (from `haptic_service.dart`) and `AudioProvider` are integrated, their application is inconsistent across standard Flutter widgets like standard `ListTile` or `GestureDetector` that aren't wrapped in `CozyButton`. Accessibility is lacking on several custom icons (e.g., `CozyHubButton` does have a tooltip, but other icon-only components like `floating_medical_icons.dart` or HUD overlays in `cozy_actions_overlay.dart` need stricter tooltip adherence).
...
*   *Issue:* Running the 3D/Isometric `room_screen.dart` behind the Quiz Session increases visual noise and drains battery.
...
*   *Rationale:* Reduces cognitive overload during high-stress activities (answering board-style questions) and significantly improves performance.
*   *Reference:* See `WIREFRAMES/quiz_session.svg` for the focused layout.

**Medium Priority: Centralized Quick-Action HUD**
*   *Issue:* Users must pan around the 3D room to find specific modules (Shop, Friends, Settings). The `cozy_actions_overlay.dart` currently floats above the view but can feel disconnected.
*   *Solution:* Introduce a persistent, collapsible 2D HUD at the bottom of the room containing quick-access icons to major app sections. We should also group actions in `cozy_actions_overlay.dart` logically.
...
*   *Solution:* Audit all `GestureDetector` and `InkWell` widgets in the app. Ensure any button or card that changes state triggers a `lightTap()` or `mediumTap()` along with the corresponding audio SFX. Enhance `cozy_button.dart` and `pressable_mixin.dart` usages.
*   *Rationale:* Essential for the "Cozy" tactile feel the brand promises.

**Medium Priority: Enhance Accessibility with Tooltips**
*   *Issue:* Icon-only interactive widgets may be confusing for new users and lack accessibility semantic labels.
*   *Solution:* Wrap icon buttons and `floating_medical_icons.dart` with Flutter's `Tooltip` widget. Ensure semantic labels are used to describe actions like "Network", "Equip", "Settings" in `cozy_actions_overlay.dart`.
*   *Rationale:* Provides semantic context for screen readers and helpful text on hover for web/desktop users.
...
*   **Recommendation:**
    ...
    *   **Subdomain Strategy:**
        ...
        *   `admin.arbormed.com`: Dedicate this subdomain to the Admin view to keep administrative traffic isolated and secure.
