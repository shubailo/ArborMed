...
*   **Visibility of System Status:**
    *   *Positive:* Gamification elements like coins and streaks update dynamically (`CoinParticle`, `ConfettiOverlay`).
...
*   **Match Between System and Real World:**
    *   *Positive:* The "Shop" and terminology ("Clinic") cleverly match the medical student reality while keeping it playful.
*   **Consistency and Standards:**
    *   *Negative:* The mixture of modal/overlay interactions vs. full-screen routing for the room screen creates navigation confusion. For instance, the transition from Dashboard to Quiz sometimes layers heavy 3D elements behind the quiz, distracting from the cognitive load of studying.
*   **Aesthetic and Minimalist Design:**
    *   *Negative:* The isometric room (`room_screen.dart`), while central to the "Cozy Competence" theme, is computationally and visually heavy when running beneath intensive tasks like Duel Mode.

### 2.2 Content and Architecture
*   **Information Architecture:** The navigation heavily relies on contextual sheets (e.g., `ContextualShopSheet`, `ClinicDirectorySheet`) invoked from a 3D hub. While immersive, it obscures direct paths to high-yield actions (like "Resume Last Study Session").
*   **Content Organization:** The Quiz interface (`QuizSessionScreen`) correctly places the stem (question text) in prominent focus, but the answer option hit targets and feedback overlays (`QuizFeedbackOverlay`) occasionally overlap with floating decorative particles, creating visual clutter during the crucial "learning from mistakes" phase.

### 2.3 Visual Design
*   **Color & Typography:** The pastel palette (Sage greens `#8CAA8C`, soft clays `#C48B76`, ivory backgrounds `#FDFCF8`) strictly adheres to the "Cozy Competence" guidelines defined in `LightPalette` and `DarkPalette`. The use of `GoogleFonts.figtree` and `GoogleFonts.notoSans` is modern and readable.
...
*   *Issue:* Running the 3D/Isometric room screen behind the `QuizSessionScreen` increases visual noise and drains battery.
...
*   *Solution:* Audit all `GestureDetector` and `InkWell` widgets in the app. Ensure any button or card that changes state triggers `CozyHaptics.lightTap()` or `CozyHaptics.mediumTap()` along with the corresponding audio SFX.
...
*   **Recommendation:**
    *   **Primary Domain:** `arbormed.ai` and `arbor-med.netlify.app` should serve as the primary access points.
    *   **Subdomain Strategy:**
        *   `arbor-med.netlify.app`: Host the Flutter Web build here for seamless browser access.
        *   `arbormed.ai`: Core domain for backend services and API, matching the existing email architecture (`hemmy@arbormed.ai`).
...
3.  **Collaborative Study Rooms (Social Extension):**
    *   Allow players to invite friends to their custom isometric room. While hanging out, they can trigger synchronous "Flashcard Marathons" using the existing Socket.IO Duel Mode infrastructure, but in a cooperative mode.
