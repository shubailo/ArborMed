...
*   **Aesthetic and Minimalist Design:**
    *   *Negative:* The isometric room (`RoomWidget`), while central to the "Cozy Competence" theme, is computationally and visually heavy when running beneath intensive tasks like timed ECG practice or Duel Mode.
...
### 2.3 Visual Design
*   **Color & Typography:** The pastel palette (Sage greens `#8CAA8C`, warm browns `#D2B48C`, creamy backgrounds `#F4F1ED`) strictly adheres to the "Cozy Competence" guidelines.
...
*   *Solution:* Introduce a persistent, collapsible 2D HUD at the bottom of the `RoomWidget` containing quick-access icons to major app sections (e.g., `ClinicDirectorySheet`).
*   *Rationale:* Balances the immersive 3D exploration with the practical need for fast navigation.
*   *Reference:* See `WIREFRAMES/dashboard.svg` for the HUD layout.

**Medium Priority: Revamp Gamification Overlays**
*   *Issue:* `ConfettiOverlay` and `CoinParticle` elements obscure the `QuizFeedbackOverlay` during learning.
*   *Solution:* Restrict particle effects to a dedicated "reward zone" at the top or bottom of the screen, ensuring the question stem and explanations remain 100% visible.
*   *Rationale:* Prevents gamification from hindering the actual educational value of reviewing mistakes.

**Low Priority: Standardize Haptic & Audio Feedback**
*   *Issue:* Inconsistent application of `CozyHaptics` and `AudioProvider`.
*   *Solution:* Audit all interactive widgets in the app. Ensure any button or card that changes state triggers appropriate haptics and audio SFX.
...
1.  **"Focus Mode" Study Timer:**
    *   Integrate a Pomodoro-style timer directly into the Study Dashboard. When activated, the `RoomWidget` lights dim, background lo-fi music starts via `AudioProvider`, and notifications are muted.
2.  **Peer-to-Peer Collaborative Study Pods:**
    *   Allow users to invite friends (via `ClinicDirectorySheet`) to their custom isometric room. While hanging out, they can trigger synchronous "Flashcard Marathons" or ECG reviews (`ecg_practice_screen.dart`) in a cooperative mode.
3.  **Dynamic Spaced-Repetition Visualizer:**
    *   A new section in the profile that visually maps out a student's memory decay curve and upcoming reviews, styled like a patient's vital signs monitor.
4.  **Drag-and-Drop Room Editing:**
    *   Upgrade the `ContextualShopSheet` to allow users to physically drag items from the MEDICAL SUPPLY DISPATCH TERMINAL directly into the `RoomWidget`, rather than relying solely on "Equip" buttons.