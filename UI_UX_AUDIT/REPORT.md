...
*   **Visibility of System Status:**
    ...
    *   *Negative:* When loading large question banks locally via `Drift`, the Quiz Loading Screen lacks sufficient granular progress communication, sometimes appearing frozen.
...
*   **User Control and Freedom:**
    *   *Negative:* The navigation lacks a clear persistent "Back" or "Home" escape hatch when deep within contextual sheets.
...
*   **Error Prevention and Recovery:**
    *   *Negative:* Interactive elements built using custom `GestureDetector` widgets do not consistently provide clear failure states or semantic labels for screen readers, making recovery difficult for users relying on assistive technologies.

### 2.2 Content and Architecture
*   **Information Architecture:** The navigation heavily relies on contextual sheets invoked from a 3D hub. While immersive, it obscures direct paths to high-yield actions (like "Resume Last Study Session").
*   **Content Organization:** The Quiz interface correctly places the stem (question text) in prominent focus, but the answer option hit targets and feedback overlays occasionally overlap with floating decorative particles (like `ConfettiOverlay` or `CoinParticle`), creating visual clutter during the crucial "learning from mistakes" phase.

### 2.3 Visual Design
*   **Color & Typography:** The pastel palette (Sage greens `#8CAA8C`, warm browns, creamy backgrounds `#F4F1ED`) strictly adheres to the "Cozy Competence" guidelines. The use of `GoogleFonts.figtree` is modern and readable.
*   **Interactivity:** Interactive elements lack sufficient tactile feedback natively. Although Haptic and `AudioProvider` are integrated, their application is inconsistent across standard Flutter widgets like standard `ListTile` or `GestureDetector`.

### 2.4 Accessibility
*   **Semantic Layer:** The application frequently relies on `GestureDetector` for custom interactive UI elements. Currently, many of these lack appropriate semantic wrappers, meaning they do not inherently expose button semantics to screen readers.
*   **Tooltips and Affordances:** Some interactive UI elements such as pagination controls and close buttons use hardcoded or generic tooltips instead of leveraging standard, localized elements, creating inconsistencies for assistive users.
...
**High Priority: Improve Accessibility (A11y)**
*   *Issue:* Interactive elements built using `GestureDetector` omit necessary accessibility hooks. Tooltips on standard controls are ad-hoc.
*   *Solution:* Wrap custom interactive elements in `Semantics(button: true, label: ...)` widgets. For standard elements like pagination controls and close buttons, use `MaterialLocalizations.of(context)` (e.g., `.previousPageTooltip`, `.closeButtonTooltip`) to provide localized, consistent tooltips.
*   *Rationale:* Meeting accessibility standards guarantees all medical students, regardless of ability, can navigate the application efficiently.
...
*   *Issue:* Inconsistent application of Haptic and audio cues across interactive elements.
*   *Solution:* Audit all `GestureDetector` and `InkWell` widgets in the app. Ensure any button or card that changes state triggers an impact along with the corresponding audio SFX.
...
*   *Reference:* See `WIREFRAMES/shop.svg` for the refined empty state layout.
...
*   **Recommendation:**
    ...
    *   **Subdomain Strategy:**
        ...
        *   `admin.arbormed.com`: Dedicate this subdomain to the admin shell to keep administrative traffic isolated and secure.
