...
*   **Information Architecture:** The navigation heavily relies on contextual sheets invoked from a 3D hub (`RoomWidget`). While immersive, it obscures direct paths to high-yield actions (like "Resume Last Study Session").
...
*   *Solution:* Implement a solid, themed background (e.g., `#F4F1ED` with subtle watermark patterns) for the Quiz Session. Specifically, ensure that `QuizSessionScreen` utilizes an opaque `Scaffold` background, fully occluding `RoomWidget` to prevent it from rendering. Floating decorative particles like `ConfettiOverlay` and `CoinParticle` should not overlap active learning material.
...
*   *Issue:* Users must pan around the 3D room to find specific modules (Shop, Friends, Settings). The current `CozyActionsOverlay` floats `CozyHubButton` icons randomly on the sides, obscuring the room.
*   *Solution:* Introduce a persistent, collapsible 2D HUD at the bottom of the `RoomWidget` (updating `CozyActionsOverlay`) containing quick-access icons to major app sections. `StartSessionHero` should be repurposed as a primary FAB centered above this new bottom HUD.
...
*   *Issue:* Inconsistent application of `CozyHaptics` and audio cues across interactive elements. Widgets like `StartSessionHero` lack haptic feedback, while `CozyHubButton` manually triggers `AudioProvider` but omits `CozyHaptics`.
*   *Solution:* Audit all `GestureDetector` and `InkWell` widgets in the app. Ensure any button or card that changes state triggers `CozyHaptics.lightTap()` or `CozyHaptics.mediumTap()` along with the corresponding audio SFX from `AudioProvider`.
...
*   **Recommendation:**
    ...
    *   **Subdomain Strategy:**
        ...
        *   `admin.arbormed.com`: Dedicate this subdomain to the `AdminShell` to keep administrative traffic isolated and secure.
