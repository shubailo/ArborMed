...
exports.getQuestionStats = catchAsync(async (req, res, next) => {
  ...
  // ⚡ Bolt: Pre-aggregate responses to prevent join explosion while maintaining predicate pushdown
  const query = `
        WITH agg_responses AS (
            SELECT
                question_id,
        ...
        SELECT 
            ...
            COALESCE(r.total_attempts, 0) as total_attempts,
            COALESCE(r.correct_count, 0) as correct_count,
            COALESCE(r.avg_time_ms, 0) as avg_time_ms
        FROM questions q
        LEFT JOIN topics t ON q.topic_id = t.id
        LEFT JOIN agg_responses r ON r.question_id = q.id
