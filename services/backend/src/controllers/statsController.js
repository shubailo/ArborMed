...
exports.getQuestionStats = catchAsync(async (req, res, next) => {
  ...
  // What: Replaced direct LEFT JOIN on responses with a pre-aggregated CTE.
  // Why: Joining responses directly with questions and topics and doing a global grouping causes an O(N*M) row explosion, drastically slowing down the query. Isolating the responses aggregation to a CTE avoids this.
  // Impact: Reduces query execution time by minimizing row operations computed by the DB engine before joining.
  // Measurement: Verification via DB performance monitoring.
  const query = `
        ...
        SELECT 
            ...
            ROUND(r.avg_time_ms)::int as avg_time_ms
        FROM questions q
