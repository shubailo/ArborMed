...
exports.getQuestionStats = catchAsync(async (req, res, next) => {
  ...
  // ⚡ Bolt: Query Optimization - Pre-aggregated responses data using LATERAL join to prevent O(N*M) memory explosion
  const query = `
        SELECT 
        ...
        ORDER BY COALESCE(r.total_attempts, 0) DESC, question_text ASC
    `;
