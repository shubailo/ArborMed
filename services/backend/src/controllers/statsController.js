...
exports.getQuestionStats = catchAsync(async (req, res, next) => {
  ...
  // What: Moved responses aggregation into a filtered CTE
  // Why: Aggregating responses independently before joining prevents large data sets from being joined and then grouped, minimizing memory allocation. Also utilizing the existing \`reponseTopicFilter\` inside the CTE significantly limits the rows scanned when filtering by a specific topic.
  // Impact: Decreases query time by limiting row evaluation during aggregation and joins.
  // Measurement: Verify execution time via Postgres EXPLAIN ANALYZE on queries filtering by topic.
  const query = `
        WITH PreAggregatedResponses AS (
            SELECT
                r.question_id,
        ...
        SELECT 
            ...
            COALESCE(ROUND(r.avg_time_ms), 0)::int as avg_time_ms
        FROM questions q
        LEFT JOIN topics t ON q.topic_id = t.id
        LEFT JOIN PreAggregatedResponses r ON r.question_id = q.id
