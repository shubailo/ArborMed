...
function buildReadinessResponse(metrics) {
  ...
  return {
    overallReadiness:
      metrics.length > 0 ? Math.round(total / metrics.length) : 0,
    breakdown: metrics
      .map((m) => ({
        topic: m.topic,
        slug: m.slug,
        score: m.readiness,
        metrics: { mastery: m.mastery, retention: m.retention },
      }))
...
exports.getQuestionStats = catchAsync(async (req, res, next) => {
  ...
  // What: Replaced direct LEFT JOIN on the responses table with a CTE (pre_agg_responses).
  // Why: Joining the responses table (1-to-many) directly to questions and topics before grouping causes a large cross-product row explosion in memory, drastically slowing down DB performance.
  // Impact: Reduces query execution time and memory allocations by aggregating responses by question_id first.
  // Measurement: Verify DB execution time via query planner or performance monitoring under load.
  const query = `
        WITH pre_agg_responses AS (
            ...
            FROM responses r
            WHERE 1=1 ${topicId ? reponseTopicFilter : ''}
        ...
        SELECT 
            ...
            COALESCE(par.total_attempts, 0)::int as total_attempts,
            COALESCE(par.correct_count, 0)::int as correct_count,
            ROUND(par.avg_time_ms)::int as avg_time_ms
        FROM questions q
        LEFT JOIN topics t ON q.topic_id = t.id
        LEFT JOIN pre_agg_responses par ON par.question_id = q.id
  ...
  const [result, userResults, bloomResult, trendResults] = await Promise.all([
    ...
    db.query(
      `
            ...
            WHERE NOT EXISTS (SELECT 1 FROM users u WHERE u.id = qs.user_id AND u.email IN ('test_reset@example.com', 'hemmy@arbormed.ai', 'endre@medbuddy.ai'))
            ${topicId ? `AND EXISTS (SELECT 1 FROM responses r WHERE r.session_id = qs.id ${reponseTopicFilter})` : ''}
        `,
      params
    ),
    db.query(
      `
            ...
            FROM user_topic_progress utp
            WHERE NOT EXISTS (SELECT 1 FROM users u WHERE u.id = utp.user_id AND u.email IN ('test_reset@example.com', 'hemmy@arbormed.ai', 'endre@medbuddy.ai'))
            ${topicId ? `AND EXISTS (SELECT 1 FROM topics WHERE topics.slug = utp.topic_slug AND (id = $1 OR parent_id = $1))` : ''}
        `,
      params
    ),
    db.query(
      `
            ...
            AND NOT EXISTS (SELECT 1 FROM users u WHERE u.id = s.user_id AND u.email IN ('test_reset@example.com', 'hemmy@arbormed.ai', 'endre@medbuddy.ai'))
            ${topicId ? topicFilter : ''}
        `,
      params
    ),
...
exports.getInventorySummary = catchAsync(async (req, res) => {
  const query = `
        SELECT 
            ...
            c.id as section_id,
            COALESCE(c.name_en, c.name_hu) as section_name,
            COALESCE(q.bloom_level, q.difficulty, 1) as bloom_level,
...
exports.getSmartReview = catchAsync(async (req, res) => {
  try {
    ...
    const reviewList = metrics
      .filter((m) => m.retention < 85)
