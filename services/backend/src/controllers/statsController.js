...
exports.getQuestionStats = catchAsync(async (req, res, next) => {
  ...
  // What: Replaced direct LEFT JOINs to `responses` with CTEs that pre-aggregate data by question_id first.
  // Why: Joining `questions` directly to `responses` and grouping at the top level causes an O(N*M) row explosion in memory, drastically slowing down the query. Isolating the `responses` aggregation to a CTE prevents this and speeds up execution significantly.
  ...
  const query = `
        ...
        SELECT 
            ...
            COALESCE(ar.total_attempts, 0)::int as total_attempts,
            COALESCE(ar.correct_count, 0)::int as correct_count,
            COALESCE(ROUND(ar.avg_time_ms), 0)::int as avg_time_ms
        FROM questions q
        LEFT JOIN topics t ON q.topic_id = t.id
        LEFT JOIN PreAggregatedResponses ar ON ar.question_id = q.id
