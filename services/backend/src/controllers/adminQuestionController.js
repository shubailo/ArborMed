...
exports.adminGetQuestions = catchAsync(async (req, res, next) => {
  ...
  // Also prevent prototype pollution by validating `sortBy` is string and using `hasOwnProperty`
  ...
  const orderBy = (sortBy && typeof sortBy === 'string') ? sortMap[sortBy] : 'q.created_at';

  // 🛡️ Sentinel: Validate `order` is a string before calling toUpperCase() to prevent TypeError DoS
  // when `order` is provided as an array
