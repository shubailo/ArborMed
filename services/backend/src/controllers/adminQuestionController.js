...
exports.adminBulkAction = catchAsync(async (req, res, next) => {
  ...
  const isSuperAdmin = req.user.email === process.env.SUPER_ADMIN_EMAIL;
  if (!isSuperAdmin) {
    // ⚡ Bolt: Bulk Authorization Validation
    // What: Replaced fetching all rows and validating in JS with a single negated SQL query using LIMIT 1.
    // Why: When validating bulk operations, fetching and iterating over all items introduces unnecessary data transfer and processing overhead. Letting PostgreSQL fail-fast with a negated check minimizes memory footprint and O(N) loops.
    // Impact: Avoids array iteration and large payload transfers, reducing response times for large bulk actions.
    // Measurement: Compare memory usage and execution time of bulk action endpoint with 100+ questions.
