...
exports.adminGetQuestions = catchAsync(async (req, res, next) => {
  ...
  // 🛡️ Sentinel: Strict validation for SQL injection prevention on ORDER BY & mitigate DoS/Prototype pollution
  if (sortBy !== undefined && typeof sortBy !== 'string') {
    return next(new AppError('Invalid sort parameter format', 400));
  }
  ...
  if (order !== undefined && typeof order !== 'string') {
    return next(new AppError('Invalid order parameter format', 400));
  }
