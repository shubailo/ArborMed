...
function validateBilingual(userAnswer, dbCorrectAnswer, options) {
  ...
  if (options && options.en && options.hu) {
    ...
    if (correctIndices.length > 0) {
      ...
      const mappedCorrect = correctIndices.map(
        (idx) =>
          resultList[idx] || (isUserHu ? options.en[idx] : options.hu[idx])
      );
      normalizedCorrect =
        mappedCorrect.length > 1 ? mappedCorrect : mappedCorrect[0];
    } else {
    ...
    isCorrect =
      cNorms.length === uNorms.length &&
      cNorms.every((c) => uNorms.includes(c));
    normalizedCorrect = cNorms.length > 1 ? cNorms : cNorms[0];
  }
