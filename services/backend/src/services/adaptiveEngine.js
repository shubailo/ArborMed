...
class AdaptiveEngine {
    ...
    async processAnswerResult(userId, topicSlug, isCorrect, questionId, _bloomLevel = 1, quality = null) {
        ...
        // We also need to persist predictive_mastery_boost since it might have been updated by Nexus Boost
        await db.query(`
            UPDATE user_topic_progress
            SET current_bloom_level = $1, current_streak = $2, level_correct_count = $3, 
                mastery_score = $4, predictive_mastery_boost = $5, last_studied_at = NOW()
            WHERE user_id = $6 AND topic_slug = $7
        `, [current_bloom_level, current_streak, level_correct_count, mastery_score, predictive_mastery_boost, userId, topicSlug]);
