...
class AdaptiveEngine {
  ...
  async getNextQuestion(
    ...
    if (levelOverride !== null) {
      console.log(
        `[PREDICTIVE] Fetching Level ${levelOverride} question for User ${userId}`
      );
    ...
    if (dueReview.rows.length > 0) {
      console.log(
        `[SRS] Serving Review Question for User ${userId}: ${dueReview.rows[0].id}`
      );
  ...
  async processAnswerResult(
    ...
    if (progressRes.rows.length === 0) {
      console.log(
        `[ADY] Initializing missing progress for User ${userId} on ${topicSlug}`
      );
      await db.query(
        `
                INSERT INTO user_topic_progress (user_id, topic_slug, current_bloom_level)
    ...
    stability = analyticsEngine.calculateNewStability(
      stability,
      bloomLevel,
      isCorrect
    );
    ...
    const totalTopicCount =
      parseInt(progressStats.rows[0].total_topic_questions) || 1;

    const masteryPoints = masteredCount * 1.0 + learningCount * 0.5;
    const mastery_score = Math.min(
      100,
      Math.round((masteryPoints / totalTopicCount) * 100)
    );
    ...
    if (isCorrect) {
      ...
      const masteredInLevel =
        parseInt(levelStats.rows[0].mastered_in_level) || 0;
      ...
      if (
        (coverage >= MASTERY_THRESHOLD ||
          level_correct_count >= STREAK_THRESHOLD) &&
        current_bloom_level < MAX_BLOOM_LEVEL &&
        nextLevelCount > 0
      ) {
  ...
  async updateSRS(userId, questionId, isCorrect) {
    ...
    console.log(
      `[SRS] User ${userId} Q ${questionId}: Box ${box} -> ${newBox} | Res ${isCorrect ? 'OK' : 'X'} | Strk ${consecutive} | Mastered: ${isMastered} `
    );
  }
}
