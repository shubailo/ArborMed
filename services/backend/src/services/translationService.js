  ...
  try {
    ...
    if (questionData.options && Array.isArray(questionData.options)) {
      translated.options = await Promise.all(
        questionData.options.map((o) => safeTrans(o))
      );
    }
    ...
    if (questionData.lefts && Array.isArray(questionData.lefts)) {
      translated.lefts = await Promise.all(
        questionData.lefts.map((l) => safeTrans(l))
      );
    }
    if (questionData.rights && Array.isArray(questionData.rights)) {
      translated.rights = await Promise.all(
        questionData.rights.map((r) => safeTrans(r))
      );
    }
