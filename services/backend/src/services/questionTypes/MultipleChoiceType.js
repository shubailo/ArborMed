...
class MultipleChoiceType extends QuestionType {
  constructor() {
    super(
      'multiple_choice',
      'Multiple Choice',
      'Choose one or more correct answers from multiple options'
    );
  }
  ...
  validate(questionData) {
    ...
    if (
      ...
      errors.push(
        'At least 2 English options are required in content.options.en'
      );
    }
    ...
    if (
      ...
      try {
        ...
        if (!Array.isArray(correctArr) || correctArr.length === 0) {
          ...
          if (invalid.length > 0) {
            errors.push(
              `Some correct answers are not in the options list: ${invalid.join(', ')}`
            );
          }
        }
      } catch {
  ...
  checkAnswer(question, userAnswer) {
    ...
    return {
      ...
      score: isCorrect ? 1 : 0,
      feedback: isCorrect
        ? 'Helyes! Minden jó választ megtaláltál.'
        : `Sajnos nem találtad el az összeset. A helyes válaszok: ${correctArr.join(', ')}`,
    };
  }
