...
class MatchingType extends QuestionType {
  ...
  checkAnswer(question, userAnswer) {
    ...
    try {
      ...
      if (Object.keys(userPairs).length !== leftKeys.length) {
        ...
        for (const leftEn of leftKeys) {
          if (
            String(userPairs[leftEn]).trim() !==
            String(correctPairs[leftEn]).trim()
          ) {
            isCorrect = false;
            break;
          }
    ...
    return {
      ...
      score: isCorrect ? 1 : 0,
      feedback: isCorrect
        ? 'Zseniális! Minden párosítás helyes.'
        : 'Sajnos nem minden párosítás volt korrekt.',
    };
  }
  ...
  prepareForClient(question) {
    ...
    if (question.content && Array.isArray(question.content.pairs)) {
      ...
      const leftItems = question.content.pairs
        .map((p) => p.left)
        .sort(() => Math.random() - 0.5);
      const rightItems = question.content.pairs
        .map((p) => p.right)
        .sort(() => Math.random() - 0.5);
