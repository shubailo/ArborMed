...
class RelationAnalysisType extends QuestionType {
  constructor() {
    super(
      'relation_analysis',
      'Relation Analysis',
      'Analyze the relationship between two medical statements'
    );
  }
  ...
  validate(questionData) {
    ...
    if (!questionData.correct_answer) {
      ...
      !validAnswers.includes(String(questionData.correct_answer).toUpperCase())
    ) {
  ...
  prepareForClient(question) {
    ...
    clientQuestion.options = [
      {
        value: 'A',
        label: 'Mindkét állítás igaz, és van köztük ok-okozati összefüggés',
      },
      {
        value: 'B',
        label: 'Mindkét állítás igaz, de nincs köztük összefüggés',
      },
