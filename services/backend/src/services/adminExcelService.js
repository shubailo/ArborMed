...
class AdminExcelService {
  ...
  async generateTemplate() {
    ...
    const subjectsResult = await db.query(
      'SELECT id, name_en FROM topics WHERE parent_id IS NULL ORDER BY name_en ASC'
    );
    const subjects = subjectsResult.rows;

    const topicsResult = await db.query(
      'SELECT id, name_en, parent_id FROM topics WHERE parent_id IS NOT NULL ORDER BY name_en ASC'
    );
    ...
    subjects.forEach((s, i) => {
      ...
      if (subjectTopics.length > 0) {
        workbook.definedNames.add(
          sanitizedName,
          `Reference!$${colLetter}$1:$${colLetter}$${subjectTopics.length}`
        );
      } else {
        ...
        workbook.definedNames.add(
          sanitizedName,
          `Reference!$${colLetter}$1:$${colLetter}$1`
        );
      }
    });
    ...
    workbook.definedNames.add(
      'SubjectsList',
      `Reference!$A$1:$A$${subjects.length}`
    );
      ...
      const fields = [
        {
          label: 'Subject',
          key: 'subject',
          validation: { type: 'list', formulae: ['SubjectsList'] },
        },
        { label: 'Topic (Section)', key: 'topic' }, // Will handle dynamic validation later
        {
          label: 'Bloom Level',
          validation: { type: 'list', formulae: ['"1,2,3,4,5,6"'] },
        },
        {
          label: 'Type',
          validation: {
            type: 'list',
            formulae: [
              '"single_choice,multiple_choice,true_false,matching,ordering,case_study"',
            ],
          },
        },
  ...
  async parseFile(buffer, mimeType) {
    ...
    if (this.isValidExcelFile(buffer, mimeType)) {
      ...
      const listSheet =
        workbook.getWorksheet('Questions List') ||
    ...
    rows.forEach((r, idx) => {
      ...
      if (!groups[key]) {
        groups[key] = {
          ...r,
          topic_id: topicMap[r.topic?.toString().toLowerCase()] || 0,
        };
      } else {
        const g = groups[key];
        ...
        if (!g.topic_id)
          g.topic_id = topicMap[r.topic?.toString().toLowerCase()] || 0;
      }
    });
