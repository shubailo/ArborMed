...
async function backfillAnalytics() {
  try {
    ...
    for (const row of topics.rows) {
      ...
      const { total, correct, sessions, earned_bloom, total_bloom } =
        stats.rows[0];

      // Calculate Mastery Score (Weighted Percentage)
      const mastery =
        total_bloom > 0 ? Math.round((earned_bloom / total_bloom) * 100) : 0;

      console.log(
        `Backfilling User ${user_id} - ${topic_slug}: ${mastery}% Mastery (${correct}/${total})`
      );
