...
async function cleanupPhase2() {
  ...
  try {
    ...
    const caseStudyIdsRes = await pool.query(
      "SELECT id FROM questions WHERE type = 'case_study'"
    );
    const caseStudyIds = caseStudyIdsRes.rows.map((r) => r.id);

    if (caseStudyIds.length > 0) {
      await pool.query('DELETE FROM responses WHERE question_id = ANY($1)', [
        caseStudyIds,
      ]);
      await pool.query(
        'DELETE FROM question_performance WHERE question_id = ANY($1)',
        [caseStudyIds]
      );
      ...
      console.log(
        `Successfully deleted ${delCaseRes.rowCount} 'case_study' questions.`
      );
    } else {
    ...
    const updateMatchRes = await pool.query(
      "UPDATE questions SET type = 'matching' WHERE type = 'match'"
    );
    console.log(
      `Successfully updated ${updateMatchRes.rowCount} questions from 'match' to 'matching'.`
    );
    ...
    if (tfIds.length > 0) {
      ...
      console.log(
        `Total True/False: ${tfIds.length}. Deleting ${countToDelete}...`
      );
      await pool.query('DELETE FROM responses WHERE question_id = ANY($1)', [
        idsToDelete,
      ]);
      ...
      const delTFRes = await pool.query(
        'DELETE FROM questions WHERE id = ANY($1)',
        [idsToDelete]
      );
      console.log(
        `Successfully deleted ${delTFRes.rowCount} 'true_false' questions.`
      );
    } else {
