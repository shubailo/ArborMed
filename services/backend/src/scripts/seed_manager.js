...
function printHelp() {
  ...
  Object.keys(SCRIPT_MAP).forEach((mode) => {
    console.log(
      `  ${colors.cyan}${mode.padEnd(15)}${colors.reset} -> Runs: ${SCRIPT_MAP[mode].join(', ')}`
    );
  });
  console.log(
...
function runScript(scriptName) {
  ...
  if (!fs.existsSync(scriptPath)) {
    console.error(
      `${colors.red}Error: Script not found at ${scriptPath}${colors.reset}`
    );
    return false;
  }
...
async function main() {
  ...
  console.log(
    `${colors.bright}Starting Seed Sequence for mode: ${mode}${colors.reset}\n`
  );
  ...
  for (const script of scripts) {
    ...
    if (!success) {
      console.error(
        `${colors.red}Sequence aborted due to error.${colors.reset}`
      );
      process.exit(1);
    }
  }
