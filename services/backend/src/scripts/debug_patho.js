    ...
    try {
        ...
        if (parentRes.rows.length === 0) {
            ...
            if (sectionsRes.rows.length > 0) {
                ...
                // Check active status
                const qActive = await pool.query("SELECT active, COUNT(*) FROM questions WHERE topic_id = ANY($1) GROUP BY active", [sectionIds]);
                console.log('Active status counts:', JSON.stringify(qActive.rows, null, 2));
            }
        }
