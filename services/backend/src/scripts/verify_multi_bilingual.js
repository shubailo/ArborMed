...
async function test() {
  try {
    ...
    const res = await db.query(
      "SELECT id, correct_answer, options FROM questions WHERE question_type = 'multiple_choice' LIMIT 1"
    );
    ...
    try {
      dbCorrectArr =
        typeof dbCorrect === 'string' && dbCorrect.startsWith('[')
    ...
    const huOptsLower = options.hu
      ? options.hu.map((o) => String(o).trim().toLowerCase())
      : [];

    const correctIndices = cNorms
      .map((c) => enOptsLower.indexOf(c))
      .filter((idx) => idx !== -1);
    ...
    if (options.hu && options.hu.length > 0) {
      ...
      console.log(
        `Simulated User Answers (HU): ${JSON.stringify(huUserAnswers)}`
      );

      const uNorms = huUserAnswers.map((u) => String(u).trim().toLowerCase());

      const userIndices = uNorms
        .map((u) => {
      ...
      console.log(
        'No Hungarian options found for this question, skipping HU test.'
      );
    }
  } catch (e) {
