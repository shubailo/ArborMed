...
const QUEST_FILE = path.join(
  __dirname,
  '../data/questions/cardiovascular_questions.json'
);
const TOPIC_ID = 87; // Cardiovascular system
...
async function syncCardio() {
  ...
  try {
    ...
    if (qIds.length > 0) {
      console.log(
        `Found ${qIds.length} questions to remove. Cleaning up dependencies...`
      );
    ...
    console.log(
      `Final Database Count for Topic ${TOPIC_ID}: ${countRes.rows[0].count}`
    );
