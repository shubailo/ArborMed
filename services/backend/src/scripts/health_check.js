...
async function runHealthCheck() {
  ...
  try {
    ...
    if (colRes.rows.length === 0) {
      console.error(
        '❌ CRITICAL: questions table is missing an active status column.'
      );
      hasErrors = true;
    } else {
    ...
    if (missing.length > 0) {
      console.error(
        `❌ CRITICAL: Missing major subjects: ${missing.join(', ')}`
      );
      hasErrors = true;
    } else {
    ...
    if (orphanRes.rows.length > 0) {
      console.warn(
        `⚠️  WARNING: Found ${orphanRes.rows.length} potentially orphaned root topics:`
      );
      orphanRes.rows.forEach((r) =>
        console.warn(`   - ${r.name_en} (${r.slug})`)
      );
    }
    ...
    qCountRes.rows.forEach((r) => {
      ...
      console.log(
        `   ${r.subject.padEnd(20)}: ${r.q_count.toString().padEnd(5)} [${status}]`
      );
      if (
        parseInt(r.q_count) === 0 &&
        subjects.includes(r.subject.toLowerCase())
      ) {
        hasErrors = true;
      }
    });
    ...
    if (emptySecRes.rows.length > 0) {
      ...
      emptySecRes.rows.forEach((r) =>
        console.log(`   - [${r.parent}] ${r.name_en} (${r.slug})`)
      );
    }
    ...
    if (hasErrors) {
      console.log(
        '❌ Health Check FAILED. Please address the critical issues above.'
      );
      process.exit(1);
    } else {
