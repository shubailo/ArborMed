...
async function checkTopics() {
  try {
    ...
    const topicsRes = await pool.query(
      "SELECT id, name_en, slug, parent_id FROM topics WHERE slug = 'pathophysiology' OR parent_id IN (SELECT id FROM topics WHERE slug = 'pathophysiology')"
    );
    console.log('Topics found:', JSON.stringify(topicsRes.rows, null, 2));

    // Also check questions table columns and defaults
    const cols = await pool.query(
      "SELECT column_name, column_default FROM information_schema.columns WHERE table_name = 'questions'"
    );
    console.log(
      'Question columns and defaults:',
      JSON.stringify(cols.rows, null, 2)
    );

    // Check is_active status for new questions
    const qStatus = await pool.query(
      'SELECT id, topic_id, is_active FROM questions ORDER BY created_at DESC LIMIT 10'
    );
    console.log(
      'Latest questions status:',
      JSON.stringify(qStatus.rows, null, 2)
    );
    ...
    console.log(
      ...
      JSON.stringify(qCount.rows, null, 2)
    );
