...
async function fixHierarchy() {
  try {
    ...
    const subjectMapping = {
      ...
      pathology: {
        ...
        children: [
          ...
          'endocrine-soft-tissue-pathology',
    ...
    for (const [parentSlug, data] of Object.entries(subjectMapping)) {
      ...
      let parentRes = await db.query('SELECT id FROM topics WHERE slug = $1', [
        parentSlug,
      ]);
      let parentId;
      ...
      for (const childSlug of data.children) {
        let childRes = await db.query('SELECT id FROM topics WHERE slug = $1', [
          childSlug,
        ]);
        if (childRes.rows.length > 0) {
          const childId = childRes.rows[0].id;
          await db.query('UPDATE topics SET parent_id = $1 WHERE id = $2', [
            parentId,
            childId,
          ]);
