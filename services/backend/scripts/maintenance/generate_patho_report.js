...
async function generateReport() {
  try {
    ...
    const topicsRes = await db.query(
      'SELECT id, name_en, slug FROM topics WHERE parent_id = $1 ORDER BY name_en',
      [pathoId]
    );
    ...
    console.log(
      `| Topic Name | Total Qs | Bloom 1 | Bloom 2 | Bloom 3 | Bloom 4 | Bloom 5 |`
    );
    console.log(
      `|------------|----------|---------|---------|---------|---------|---------|`
    );
    ...
    for (const topic of topics) {
      ...
      console.log(
        `| ${topic.name_en.padEnd(10)} | ${total.toString().padEnd(8)} | ${b1.toString().padEnd(7)} | ${b2.toString().padEnd(7)} | ${b3.toString().padEnd(7)} | ${b4.toString().padEnd(7)} | ${b5.toString().padEnd(7)} |`
      );
    }
